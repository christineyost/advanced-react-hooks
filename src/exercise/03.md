...
- The `React.useContext()` hook can be used to share state between components.
  This hook should only be used in really specific circumstances and is not
  necessary most of the time
- To create context, we use `React.createContext()`. To give that context a
  value, we must create a `Context.Provider` element and set the value prop in
  it
- From anywhere in the tree below this provider element, we can access the
  context using `React.useContext(Context)`
- It's best to wrap the `Context.Provider` element in another component so it
  doesn't need to be used directly
- It's also nice to abstract the `React.useContext()` call out into a custom
  hook that throws an error if the context is not there (if it's trying to be
  accessed outside of the provider element), so that check doesn't need to
  happen everywhere it is used
