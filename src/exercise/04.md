...
- Use `React.useEffect()` most of the time (99%)
- But `React.useLayoutEffect()` is useful if the side effect you need to happen
  will make an observable change to the DOM (such as scrolling in this example)
- `useLayoutEffect()` runs before the browser is painted, so in the scrolling
  example, the browser will be painted with the correct scroll location
- `useEffect()` runs after the browser is painted, so if a scroll is done in
  this hook, the browser will already be painted, then the scroll location will
  be visibly updated
