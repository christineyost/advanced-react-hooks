...
- The `React.useCallback()` hook is helpful for when a function needs to be part
  of a dependency array. If the function is defined as part of the component, it
  will be redefined during every rerender, and so the effect that this
  dependency is part of will be called for every rerender. The
  `React.useCallback()` hook accepts a function and a dependency list.
  `useCallback` will only return the function as a new function if a dependency
  in the list changed
- It's better to use `React.useCallback()` in a hook or something abstracted so
  that users of a custom hook do not need to use the `React.useCallback()` hooks
  on their own functions before passing them to a custom hook
