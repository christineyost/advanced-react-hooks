...
- `React.useReducer()` is a hook that is called with two parameters, a reducer
  function and an initial value. It returns an array of two items, the current
  value and a dispatch function. When the dispatch function is called, the
  reducer function is called with the current value and whatever the dispatch
  function was called with
- The second parameter to the reducer function is refered to as the `action`
- The redux reducer pattern is one that is frequently used. See the final state
  of this first exercise for an example. This pattern does not have to be used
- Lazy initialization via a function can also be used for the initial value when
  using `React.useReducer()`. To do that, three parameters need to be passed
  instead of two. First, the reducer function (same as before). Then a second
  parameter which will be passed to the state initialization function. Third,
  the function that will return the initial state, given the second parameter
